...
template <typename T> struct std::iterator_traits<best::Iterator<T>> {
...
template <typename T> Iterator<T> &Iterator<T>::operator+=(difference_type n) {
  ptr += n;
  return *this;
}
...
Iterator<T>::operator-(const Iterator &other) const {
...
Iterator<T>::operator[](difference_type i) const {
  return ptr[i];
}
